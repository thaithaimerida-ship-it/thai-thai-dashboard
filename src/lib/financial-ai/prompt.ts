...
export function buildFinancialAISystemPrompt(): string {
  return [
    ...
    'Los bloques resumen_financiero, rentabilidad, caja_operativa y comisiones_canales son deterministas: copia sus numeros desde payload.agregados y no los recalcules.',
  ].join('\n');
}
...
export function buildFinancialAIUserPrompt(payload: FinancialAIPayload): string {
  return [
    ...
    JSON.stringify(
      {
        ...
        resumen_financiero: {
          ingresos_brutos_financieros: 0,
          comisiones_totales: 0,
          ingresos_netos_financieros: 0,
          meta_ventas_netas: 325000,
          punto_equilibrio: 295000,
          diferencia_vs_meta: 0,
          diferencia_vs_punto_equilibrio: 0,
          cumplimiento_meta_pct: 0,
          cumplimiento_pe_pct: 0,
        },
        rentabilidad: {
          food_cost_monto: 0,
          food_cost_pct: 0,
    ...
    '- Copia resumen_financiero desde payload.agregados.resumen_financiero sin cambiar los numeros.',
    '- Copia rentabilidad desde payload.agregados.rentabilidad sin cambiar los numeros.',
    '- Copia caja_operativa desde payload.agregados.caja_operativa sin cambiar los numeros.',
    '- Copia comisiones_canales desde payload.agregados.comisiones_canales sin cambiar los numeros.',
    '- No omitas KPIs obligatorios: ingresos netos financieros, meta mensual, punto de equilibrio, food cost, labor cost, costo primo, cash yield si esta disponible, venta con impuesto, impuesto total, venta neta caja, comensales y ticket promedio.',
